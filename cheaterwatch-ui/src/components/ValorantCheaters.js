...
const ValorantCheaters = () => {
  ...
  const gameName = 'Valorant'; // Game name to filter cheaters for Valorant
  ...
  useEffect(() => {
    ...
    const fetchAllReports = async () => {
      try {
        ...
        const valorantReports = response.data.filter(report => report.game === gameName);
        setReports(valorantReports);
        setFilteredReports(valorantReports);
      } catch (error) {
...
export default ValorantCheaters;