...
const WarzoneCheaters = () => {
  ...
  const gameName = 'Call of Duty Warzone'; // Game name to filter cheaters for Warzone
  ...
  useEffect(() => {
    ...
    const fetchAllReports = async () => {
      try {
        ...
        const warzoneReports = response.data.filter(report => report.game === gameName);
        setReports(warzoneReports);
        setFilteredReports(warzoneReports);
      } catch (error) {
...
export default WarzoneCheaters;
